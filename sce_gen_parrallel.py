...
def parse_args():
    parser = argparse.ArgumentParser(description="并行版 RL 场景生成训练脚本（SubprocVecEnv 多进程并行采样）")
    parser.add_argument('--gpu', type=int, default=0, help='使用哪张 GPU')
    parser.add_argument('--timesteps', type=int, default=200000, help='总训练步数')
    parser.add_argument('--seed', type=int, default=0, help='随机种子')
    parser.add_argument('--num-envs', type=int, default=None, help='并行环境数（默认=min(CPU核数, 16)）')
...
if args.num_envs is None:
    args.num_envs = min(os.cpu_count() or 1, 16)
...
# --- 1. 创建并行环境 ---
# 每个 worker 在独立子进程中采样环境，拥有独立的 HDF5 文件与独立的 numpy 随机种子，
# 避免 fork 后各进程采样出完全相同的场景。
def make_env(rank, base_seed, record_path, timestamp):
    def _init():
        # 环境内部使用全局 np.random，因此必须按 worker 显式设置随机种子
