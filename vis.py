...
def _load_episode_dict_from_json(path, key, payload):
    """Build the episode dict from a JSON payload (see export_one_episode.py).

    JSON shape: {"attrs": {...}, "trajectories": [...], "perception_data": [...]}
    where trajectories is (T,5,4) and perception_data is (T,5,13).
    """
    traj = np.asarray(payload['trajectories'], dtype=np.float32)
    perc_full = np.asarray(payload['perception_data'], dtype=np.float32)
    if traj.shape[0] == 0:
        return None
    perc = perc_full[:, :, 0:4]  # match _load_episode_dict: ego's perceived view
    ttc_true = min_ttc_vectorized(traj[:, 0, :], traj[:, 1:, :])
    ttc_perc = min_ttc_vectorized(perc[:, 0, :], perc[:, 1:, :])
    ttc_true = np.where(np.isinf(ttc_true), TTC_CAP, ttc_true)
    ttc_perc = np.where(np.isinf(ttc_perc), TTC_CAP, ttc_perc)
    return dict(
        path=path, key=key, traj=traj, perc=perc,
        ttc_true=ttc_true, ttc_perc=ttc_perc,
        collision=bool(payload.get('attrs', {}).get('collision', False)),
        reward=float(payload.get('attrs', {}).get('episode_reward', 0.0)),
    )


def load_one_episode(path, episode):
    """Load a single episode by (h5 or json path, episode id).

    Dispatches on file extension: .json uses the text payload (for environments
    where the binary h5 cannot be downloaded); everything else is treated as h5.
    """
    import json
    ...
    if path.lower().endswith(('.json', '.jsonl', '.txt')):
        with open(path, 'r') as f:
            data = json.load(f)
        # support both {"episode_80": {...}} (full export) and {...} (single)
        payload = data.get(key, data) if isinstance(data, dict) and key in data else data
        e = _load_episode_dict_from_json(path, key, payload)
        if e is None:
